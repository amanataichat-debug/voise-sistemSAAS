...
"""
🚀 PRODUCTION VERSION 1.0 - Google Gemini 3.1 Flash Live API Handler
...
from backend.websockets.gemini_client_31 import GeminiLiveClient31
...
# Active connections
active_gemini_31_connections: Dict[str, List[WebSocket]] = {}
...
def log_to_render(message: str, level: str = "INFO"):
    ...
    log_msg = f"{timestamp} - [GEMINI-31] {level} - {message}"
...
async def handle_gemini_31_websocket_connection(
    websocket: WebSocket,
    assistant_id: str,
    db: Session
) -> None:
    ...
    try:
        ...
        # Register connection
        active_gemini_31_connections.setdefault(assistant_id, []).append(websocket)
        log_to_render(f"📝 Active Gemini connections for {assistant_id}: {len(active_gemini_31_connections.get(assistant_id, []))}")
        ...
        log_to_render(f"   Model: gemini-3.1-flash-live-preview")
        ...
        await websocket.send_json({
            ...
            "status": "connected", 
            "message": "Connected to Gemini 3.1 Flash Live API",
            "model": "gemini-3.1-flash-live-preview",
            "thinking": "disabled (budget=0, optimized for voice latency)",
        ...
        gemini_task = asyncio.create_task(
            handle_gemini_31_messages(gemini_client, websocket, interruption_state)
        )
        ...
        conns = active_gemini_31_connections.get(assistant_id, [])
        if websocket in conns:
            conns.remove(websocket)
        log_to_render(f"👋 Connection closed: {client_id}")


async def handle_gemini_31_messages(
    gemini_client: GeminiLiveClient31, 
    websocket: WebSocket, 
