/**
 * Gemini 3.1 Flash Live Widget v1.0.0
 * Model: gemini-3.1-flash-live-preview
 * Endpoint: /ws/gemini-31/{assistant_id}
 * 
 * Based on Gemini Voice Widget v2.8.2
 * Purple UI theme to differentiate from 2.5 (blue)
 * 
 * @version 1.0.0
...
`;
    ...
    function init() {
        console.log('[GEMINI-31-WIDGET] 🚀 Initializing v1.0.0 (Gemini 3.1 Flash Live)...');
        ...
        const getScriptTag = () => {
            ...
            const bySrc = document.querySelector('script[src*="gemini-31-widget.js"][data-assistantId]');
        ...
        if (!scriptTag) {
            console.error('[GEMINI-31-WIDGET] Script tag with data-assistantId not found');
            return;
        }
        ...
        if (!CONFIG.assistantId || !CONFIG.serverUrl) {
            console.error('[GEMINI-31-WIDGET] Missing required parameters (assistantId or server)');
            return;
        }
        ...
        console.log('[GEMINI-31-WIDGET] ✅ Initialization complete');
    }
    ...
    async function initAudioContext() {
        ...
        console.log('[GEMINI-31-WIDGET] 🎧 Creating AudioContext...');
        ...
        console.log(`[GEMINI-31-WIDGET] AudioContext rate: ${actualRate}Hz (input will be downsampled to 16kHz in worklet)`);
    ...
    async function loadAudioWorklets() {
        try {
            ...
            console.log('[GEMINI-31-WIDGET] ✅ AudioWorklets loaded');
        } catch (error) {
            console.error('[GEMINI-31-WIDGET] ❌ AudioWorklet load failed:', error);
        }
    }
        ...
        const widgetHTML = `
            ...
            <div class="wellcomeai-widget-expanded" id="wellcomeai-widget-expanded">
                <div class="wellcomeai-widget-header">
                    <div class="wellcomeai-widget-title">Gemini 3.1 Flash Live</div>
    ...
    async function connectWebSocket() {
        ...
        const endpoint = `${wsUrl}/ws/gemini-31/${CONFIG.assistantId}`;
        ...
        try {
            ...
            STATE.ws.onopen = () => {
                console.log('[GEMINI-31-WIDGET] ✅ WebSocket connected');
            ...
            STATE.ws.onerror = (error) => {
                console.error('[GEMINI-31-WIDGET] WS Error:', error);
                updateUIState('error', 'Ошибка сервера');
            };
            
            STATE.ws.onclose = (event) => {
                console.log('[GEMINI-31-WIDGET] WS Closed:', event.code);
                STATE.isConnected = false;
                STATE.readyToRecord = false;
                stopPlayback();
    ...
    function handleWSMessage(event) {
        try {
            ...
            console.error('[GEMINI-31-WIDGET] Parse error:', e);
        }
    }
    ...
    async function startRecording() {
        ...
        console.log('[GEMINI-31-WIDGET] 🎙️ Starting recording...');
    ...
    async function stopRecording() {
        ...
        console.log('[GEMINI-31-WIDGET] 🛑 Recording stopped');
    }
