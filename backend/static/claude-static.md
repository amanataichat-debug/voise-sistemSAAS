...
- `voice-assistants.html` — **единая страница голосовых ассистентов** (дизайн-система, этап 2; спека `design-system/04-pages-cabinet.md` §2): витрина карточек OpenAI / Gemini / Fish / ElevenLabs → редактор с вкладками Настройки · Функции · База знаний · Тестирование · Встраивание. Провайдеры описаны в `PROVIDERS` (эндпоинты `/api/assistants/`, `/api/gemini-assistants`, `/api/fish-assistants`, `/api/eleven-assistants`; у Fish/Eleven список обёрнут `{assistants}`, `/options`, `/status`). Ключи OpenAI/Gemini пользователь вводит в блоке «Голосовая модель» (`PUT /api/users/me`). Смена модели у сохранённого ассистента = создание на новой модели + перепривязка его SIP-номеров (`PATCH /api/sip/numbers/{id}`) + удаление старого; при лимите — сначала удаление, при неудаче создания старый восстанавливается. Тест — тот же `widget.js`/`gemini-widget.js` (`data-server` обязателен, для Fish/Eleven `data-ws-path`); код для сайта Fish/Eleven собирается на клиенте. URL: `?id=&model=&tab=`, `?new=1&model=`. Строки — раздел `va` словарей `i18n/*.json`. Старые `agents.html`, `gemini-agents.html`, `fish-agents.html`, `eleven-agents.html` редиректятся сюда из `app.py` (`LEGACY_ASSISTANT_PAGES`, `?id` сохраняется, `?mode=create` → `?new=1`); файлы страниц остались, но не открываются. `grok-agents.html` (скрытый провайдер без SIP) по-прежнему доступен по прямой ссылке. Меню: пункт «Голосовые ассистенты» в `js/sidebar.js`.
...
- На дизайн-системе: `voice-assistants.html` — каркас A (`.vf-app`, без моста); каркас B из `design-system/03-layouts.md`
  (`<body class="vf">` + мост): dashboard, grok-agents, knowledge-base, telephony, conversations, crm, crm-contact, settings,
