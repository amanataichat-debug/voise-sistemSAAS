/**
 * VoksiAI UI — поведение компонентов дизайн-системы (v1), без зависимостей.
 *
 ...
 *   VF.loader.show()/hide()         — загрузчик страницы (волна VoksiAI); показывается сам
 ...
 *
 * Подключать после voksiai.css. Скрипт сразу вставляет загрузчик, поэтому его
...
(function (global) {
  ...
  // Строки интерфейса из словаря (js/i18n.js); без него — русские по умолчанию
  function L(key, def) { return global.I18N ? global.I18N.t(key, null, def) : def; }
  ...
  var loader = (function () {
    ...
    function ensure() {
      ...
      el.innerHTML =
        '<div class="brand"><span class="vf-wave"><i></i><i></i><i></i><i></i><i></i></span> VoksiAI</div>' +
        '<div class="sub">' + esc(L('common.loader_text', 'Загружаем кабинет…')) + '</div>';
  ...
  function toast(msg, opts) {
    ...
    t.innerHTML = icon(icons[type] || 'info') + '<span class="msg">' + esc(msg) + '</span>' +
      (opts.action ? '<button type="button" class="act">' + esc(opts.actionLabel || L('common.toast_undo', 'Отменить')) + '</button>' : '') +
      '<button type="button" class="close" aria-label="' + esc(L('common.close', 'Закрыть')) + '">' + icon('x', 'ic-sm') + '</button>';
  ...
  function confirm(opts) {
    ...
    return new Promise(function (resolve) {
      ...
      backdrop.innerHTML =
        ...
        '<div><h3>' + esc(opts.title || L('common.confirm_title', 'Подтвердите действие')) + '</h3></div></div></div>' +
        '<div class="vf-dialog-body" style="padding-left:78px">' + (opts.html || esc(opts.message || '')) + '</div>' +
        '<div class="vf-dialog-foot">' +
        '<button type="button" class="btn" data-act="cancel">' + esc(opts.cancelText || L('common.cancel', 'Отмена')) + '</button>' +
        '<button type="button" class="btn ' + (opts.danger ? 'btn-danger btn-solid' : 'btn-primary') + '" data-act="ok" autofocus>' + esc(opts.confirmText || L('common.confirm', 'Подтвердить')) + '</button>' +
        '</div></div>';
  ...
  function alertDialog(opts) {
    ...
    return new Promise(function (resolve) {
      ...
      backdrop.innerHTML =
        ...
        '<div><h3>' + esc(opts.title || L('common.alert_title', 'Сообщение')) + '</h3></div></div></div>' +
        ...
        '<div class="vf-dialog-foot"><button type="button" class="btn btn-primary" data-act="ok" autofocus>' + esc(opts.okText || L('common.ok', 'Понятно')) + '</button></div></div>';
      document.body.appendChild(backdrop);
  ...
  function select(container, opts) {
    ...
    function renderBtn() {
      ...
      var html = c ? (opts.renderValue ? opts.renderValue(c) : ((c.icon || '') + '<span class="truncate">' + esc(c.label) + '</span>' + (c.sub ? '<span class="faint small" style="margin-left:auto;padding-right:6px">' + esc(c.sub) + '</span>' : '')))
                   : '<span class="placeholder">' + esc(opts.placeholder || L('common.select_placeholder', 'Выберите…')) + '</span>';
      btn.innerHTML = html + '<span class="caret">' + icon('chevron-down', 'ic-sm') + '</span>';
    }
    function renderMenu() {
      var q = state.q.trim().toLowerCase();
      var html = '';
      if (opts.search) html += '<div class="vf-menu-search" style="position:sticky">' + icon('search') + '<input type="text" placeholder="' + esc(L('common.search_placeholder', 'Поиск…')) + '" value="' + esc(state.q) + '"></div>';
      var idx = 0, any = false;
      ...
      if (!any) html += '<div class="vf-menu-empty">' + esc(opts.emptyText || L('common.nothing_found', 'Ничего не найдено')) + '</div>';
      menu.innerHTML = html;
  ...
  function enhanceSelect(sel) {
    ...
    function placeholderOf() {
      ...
      return (first && first.value === '') ? (first.textContent || '').trim() : L('common.select_placeholder', 'Выберите…');
    }
    function refresh() {
  ...
  var FA_MAP = {
    ...
    'sync-alt': 'refresh-cw', 'sync': 'refresh-cw', 'rotate': 'rotate-ccw', 'sign-out-alt': 'log-out', 'ruble-sign': 'banknote',
