...
- **Tools агента:** `execute_tool(tool_name, tool_args, context, db)` — единая точка диспетчеризации; `context` обязан содержать `agent_config_id`, `user_id`, `user`. Исключение инструмента возвращается модели как `{"ok": false, "error": ...}`, а сессия БД откатывается (`db.rollback()`), иначе упавший commit оставлял сессию в PendingRollback и ронял весь чат. PostCall после исходящего звонка ВСЕГДА планирует следующее касание (перезвон `create_agent_task` или отложенное сообщение `schedule_telegram_message` — только при подключённом личном TG-аккаунте), кроме случая достижения цели; `fn_create_agent_task` отменяет дубли SCHEDULED-задач этого контакта (в пределах одного канала). `schedule_telegram_message` создаёт `Task(channel="telegram")`: в `description` — инструкция, текст составляется при отправке; рабочие часы не применяются.
