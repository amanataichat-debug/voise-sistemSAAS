...
| `backend/websockets/openai_client.py` | `TELEPHONY_VAD` (0.5 / 300 / 500 мс) при `assistant.telephony_mode`. Без него телефонный звонок попадал в профиль «десктоп» (0.2 / 100 / 200), потому что на звонке нет user-agent, и шум линии рвал речь ассистента ложными перебиваниями |
