...
- Полный путь доказан через софтфон: Zoiper → Asterisk → мост → Render → OpenAI/Gemini → обратно. OpenAI (Realtime): задержка ответа ~0.5 с. Gemini медленнее (модельная задержка 3–5 с) и чувствительнее к шуму линии.
