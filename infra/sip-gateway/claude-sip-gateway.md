...
хендлеры, что и браузерный виджет (`handler_live` для OpenAI на GPT-Live,
`handler_gemini` для Gemini, `handler_fish` для Fish, `handler_eleven` для Eleven), через адаптер `backend/websockets/sip_media_adapter.py`.
...
Поддерживаются ассистенты OpenAI, Gemini, Fish и Eleven (`SIP_HANDLERS` в `api/sip_gateway.py`, `SIP_SUPPORTED_ASSISTANT_TYPES` в `models/sip_gateway.py`). Входящие и исходящие звонки.
...
| Файл | Что |
|---|---|
| `install.sh` | Идемпотентный установщик/обновлятор для VPS. Тянет файлы этой папки с raw GitHub по ветке (`VOKSY_BRANCH`, по умолчанию `1409-sip-v1`), пишет `/etc/asterisk/*.conf`, ставит мост в `/opt/voksy-bridge`, генерирует секреты в `/etc/voksy-bridge/bridge.env` (не перезаписывает при повторе), ждёт `core waitfullybooted`, печатает сводку |
...
| `backend/websockets/sip_media_adapter.py` | `HandlerSocket` — псевдо-WebSocket для хендлера: ресемплинг 8↔24 кГц (OpenAI, Fish, Eleven) и 8→16 / 24→8 (Gemini) через `audioop.ratecv` **с антиалиасинговым ФНЧ 3.4 кГц** (`_LowPass`, КИХ 63 отвода на numpy, хвост переносится между кадрами): `ratecv` — линейная интерполяция без фильтра, и без ФНЧ при 24→8 кГц всё выше 4 кГц зеркалилось в голосовую полосу с той же громкостью — абонент слышал шум поверх речи; при 8→24 кГц те же призвуки слышал VAD модели. Без numpy фильтр отключается, звонок продолжает работать; батчинг входа `INBOUND_BATCH_MS` (OpenAI 20 мс, Gemini 100 мс); **запас на выходе** `OUTBOUND_CUSHION_MS` (OpenAI 200 мс): GPT-Live отдаёт аудио в реальном темпе, очередь пейсера моста почти пуста, и без запаса джиттер слышен как провалы — адаптер придерживает начало каждой реплики и потом отдаёт поток; barge-in на `speech.started`/`conversation.interrupted`/`response.cancelled` → `clear` + подкладывает хендлеру `audio_playback.stopped` (у GPT-Live таких событий нет — перебивание обрабатывает сама модель); функция `hangup_call` → дождаться прощания (`assistant.speech.started/ended`, у GPT-Live выводятся из потока аудио) → `mark` → `hangup`; замер `reply latency` (событие «пользователь договорил» → первый аудио-дельта; для GPT-Live не измеряется — нет события конца реплики абонента) |
| `backend/services/sip_gateway_service.py` | `find_number` (точное, затем по последним 9 цифрам), `queue_outbound_call`, `claim_queued_calls` (`FOR UPDATE SKIP LOCKED`), `apply_bridge_event` (статусы, requeue до 6 попыток с паузой 30 с при `RETRYABLE_FAIL_REASONS`, `_finish_task` обновляет `Task`/`AgentCall`), `tag_conversations` (проставляет `caller_number`/`direction` в `conversations`, `gemini_conversations`, `fish_conversations` или `eleven_conversations` по `assistant_type`; у Fish/Eleven точно по `session_id` = id звонка — `sip_media` задаёт `assistant.sip_session_id` и `sip_calls.conversation_session_id`, у остальных по окну времени), `resolve_greeting`, `call_context_text` |
| `backend/models/sip_gateway.py` | `SipPhoneNumber` (user_id, phone_number — только цифры, unique; gateway_id; assistant_type/assistant_id; first_phrase; allow_outbound; is_active), `SipCall` (direction, `SipCallStatus` queued/dialing/ringing/answered/completed/failed, did/caller/to_number, task_id, call_metadata JSON, trunk_host, conversation_session_id, **transcript** JSON (колонку дописывает `_ensure_tables` только если её нет, с `lock_timeout`), timestamps, duration_sec, end_reason, attempts), `normalize_sip_number()` |
| `backend/services/call_transcript.py` | `CallTranscript` — стенограмма звонка целиком по событиям хендлера (через `HandlerSocket(on_event=…)`): Fish/Eleven `input.transcription` + `response.text.*` (+ приветствие, которое озвучивается без текстовых событий), Gemini `input/output.transcription.complete`, GPT-Live `transcript.delta`. Пишется в `sip_calls.transcript` ([{role, text, t}]) в `finally` медиа-сокета; `transcript_as_text()` → «Агент: …/Клиент: …» для оркестратора. Страница «Диалоги» для звонка берёт эту стенограмму (`_sip_call_transcript` в `api/conversations.py`) |
| `backend/services/agent_call_finalizer.py` | Конец звонка → агент обзвона. `on_media_finished` (из `finally` медиа-сокета): исходящий — `AgentCall` по `source_task_id == sip_calls.task_id`; входящий на номер с `agent_config_id` — контакт (новый, если номера нет) + `AgentCall(direction="inbound")`; затем `PostCallOrchestrator.finalize_sip_call` с готовой стенограммой (answered, если абонент что-то сказал). `on_call_failed` (из `apply_bridge_event`, окончательный failed): PostCall с `no_answer`. Финализация атомарная (`_claim_for_finalization`), повтор безопасен |
| `backend/core/task_scheduler.py` | `_sip_number_for()`, `_execute_via_sip_gateway()`, `_agent_call_via_sip_gateway()`: если у пользователя есть активный номер шлюза с `allow_outbound` и ассистент OpenAI/Gemini/Fish/Eleven, запланированный звонок ставится в очередь `sip_calls`; задача агента остаётся `PENDING` до события моста, PostCall запускает `AgentCallFinalizer` по концу звонка (опрос `poll_and_run` для SIP не используется); без номера задача падает в мёртвые ветки Voximplant и завершится ошибкой |
| `backend/core/config.py` | `SIP_GATEWAY_TOKEN` (равен `GATEWAY_TOKEN` на VPS), `SIP_GATEWAY_DEFAULT_ID` (`sip-gw-1`) |
| `backend/websockets/live_client.py` | OpenAI на телефоне = GPT-Live (`gpt-live-1`): VAD нет, модель full-duplex сама решает, когда говорить; при `assistant.telephony_mode` голосовому слою добавляется `VOICE_TELEPHONY_HINT` (короче, переспрашивать). Функции (в т.ч. `hangup_call`) исполняет бэкенд делегирования (`LIVE_DELEGATION_MODEL`) на ключе владельца. Старый `TELEPHONY_VAD` в `openai_client.py`/`fish_llm_client.py` остался только для Fish |
...
- Полный путь доказан через софтфон: Zoiper → Asterisk → мост → Render → OpenAI/Gemini → обратно. OpenAI (Realtime): задержка ответа ~0.5 с. Gemini на 2.5 была медленнее (модельная задержка 3–5 с) и чувствительнее к шуму линии; с сентября 2026 Gemini работает на `gemini-3.8-live` (proactive audio включён всегда — модель сама решает, обращались ли к ней; на телефоне ещё не проверено).
- OpenAI переведён на GPT-Live (`handler_live`, сентябрь 2026); телефонный прогон на новом транспорте ещё не делался — проверить звонком: приветствие, перебивание, `hangup_call` → прощание → трубка, отсутствие провалов в звуке (запас `OUTBOUND_CUSHION_MS`).
- **Транк у оператора включён (сентябрь 2026).** Рабочий номер по карточке: `+996705701707` / `0705701707`. Каналов 5 входящих + 5 исходящих.
- Анкета оператора («Форма реализации») закрыта: наш IP `178.105.79.237`, SIP 5060/UDP, RTP 10000–20000/UDP, G.711 A-law, DTMF RFC 2833, оба SIP-сервера оператора (`195.216.237.6:5070`, `195.216.237.7:5070`), RTP оператора приходит с `195.216.237.6–.9`. Авторизация по IP, без регистрации. Исходящие — только на номера O!.
- **Формат номера на плече оператора — national (`070`/`050`).** Внутри системы номера канонические (`996705701707`), в INVITE и А-номер, и Б-номер должны быть `0705701707` / `0507909000`, иначе оператор отвечает `403 Forbidden`. Конвертирует `to_national()` в `bridge.py` только при originate.
...
- `assistant.telephony_mode = True` ставится как обычный атрибут объекта ассистента, не колонка. Его читают Fish- и Eleven-хендлеры (телефонный профиль VAD в `fish_llm_client.TELEPHONY_VAD`). Overrides приветствия/промпта делаются через `set_committed_value` при `expire_on_commit=False`, чтобы не попасть в БД.
...
## Eleven по телефону
Привязка: `{"assistant_type":"eleven","assistant_id":"<uuid>"}`. Звонок идёт через `handler_eleven` на серверных ключах `OPENAI_API_KEY`/`ELEVENLABS_API_KEY`; в логах `[ELEVEN]`, `[ELEVEN-LLM]`, `[ELEVEN-TTS]`. Аудио ElevenLabs — PCM16 24 кГц, как у Fish (ресемплинг 24→8 в адаптере). Перебивание — `close_context` на том же сокете (переподключения нет). Язык звонка — из карточки (по умолчанию кыргызский).
